...
list1 = list(map(int, input('enter time1: (hour:minute:second)\n').split(':')))
list2 = list(map(int, input('enter time2: (hour:minute:second)\n').split(':')))

t1 = Time(list1[0], list1[1], list1[2])
t2 = Time(list2[0], list2[1], list2[2])

sec = Time(0 ,0 ,int(input('enter the second : ')))


print('sum: %s\nsub: %s\ntimeToSec: %s\nsecToTime: %s'%((t1.sub(t2).show(),t1.sum(t2)).show(),t1.hourtosecond(), (sec.secondtohour()).show()))